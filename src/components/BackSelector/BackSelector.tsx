...
import cl from './BackSelector.module.css';
import type { Background } from "../types.ts";

type BackSelectorProps = {
    onCreate: (value: number) => void;
    itemList: Background[];
}
const MIDDLE_ITEM = 2;

export const BackSelector = ({ itemList, onCreate }: BackSelectorProps) => {
    ...
    const toPrevSlide = () => {
        ...
        onCreate(itemList[(currentIndex - 1 + itemList.length) % itemList.length].backgroundId);
    };

    // Функция перехода к предыдущему элементу из списка расс
    const toNextSlide = () => {
        setCurrentIndex((prev) => (prev + 1) % itemList.length);
        onCreate(itemList[(currentIndex + 1) % itemList.length].backgroundId);
    };

    const handleSlideClick = (index: number) => {
        setCurrentIndex(index);
        onCreate(itemList[index].backgroundId);
    };
                ...
                {visibleItems.map((item, i) => (
                    ...
                    <div
                        ...
                        <img
                            ...
                            alt="Изображение выбранной предыстории"
                        />
